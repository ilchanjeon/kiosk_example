...
webSocket.onmessage = async function (message) {
    ...
    if (ret.substr(4, 4) == '0000') {
        ...
        const updateEducation = await Utils.postData('/updateMonthlyRegist', {
        ...
        if(updateEducation.result == 'error' || updateEducation.result == 'No Content') {
            closeOverRay();
            showAlert("갱신 시 오류가 발생하였습니다.");
            return false;
        }
...
function Transaction(type) {
    const selected = document.querySelector('.item-box.active');
    ...
    educationNo = selected.dataset.educationno;
